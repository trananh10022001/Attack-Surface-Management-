...
# Đường dẫn nuclei
NUCLEI_PATH = 'C:\\Users\\KhanhNguyen\\go\\bin\\nuclei.exe'
...
for port in ports:
    for i in ports:
        if port['port'] == "80" and i['port'] == "443" and port['state'] == 'open':
            print(f"Port 80 and 443 are open. Scanning for vulnerabilities with Nuclei...")
            cmd = os.system(f"{NUCLEI_PATH} -target {ip}")
            # process = subprocess.run(cmd.split(), stdout=subprocess.PIPE)
            # vulnerabilities = process.stdout.decode('utf-8')
...
# 34.101.62.222 (https://oceanpianist.online/)
# 104.244.42.67
